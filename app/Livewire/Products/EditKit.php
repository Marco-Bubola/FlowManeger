...
class EditKit extends Component
{
    ...
    public function mount(Product $kit)
    {
        ...
        // Inicializar itens de custo adicional (single → repetível)
        $existingCost = number_format($kit->custos_adicionais ?? $kit->additional_costs ?? 0, 2, ',', '.');
        $this->additionalCostItems = [[
            'description' => $kit->descricao_custos_adicionais ?? $kit->additional_costs_description ?? '',
            'value' => $existingCost,
        ]];
    ...
    {
        $this->additionalCostItems[] = ['description' => '', 'value' => ''];
        $this->syncAdditionalCostsFromItems();
        $this->calculateTotals();
    }
    ...
    public function getAutoGeneratedName()
    {
        if (empty($this->selectedProducts)) {
            return $this->name ?: 'Kit Novo';
        }
        $names = [];
        $count = 0;
        foreach ($this->selectedProducts as $p) {
            if ($count >= 3) break;
            if (!empty($p['name'])) { $names[] = $p['name']; $count++; }
        }
        if (empty($names)) {
            return $this->name ?: 'Kit Novo';
        }
        if (count($this->selectedProducts) > 3) {
            return 'Kit ' . implode(' + ', $names) . ' + outros';
        }
        return 'Kit ' . implode(' + ', $names);
    }
    ...
    {
        // Fluxo de 2 etapas (igual ao create-kit):
        // Step 1 = seleção de produtos · Step 2 = configuração final
