...
class UploadCashbook2 extends Component
{
    use WithFileUploads;
    ...
    protected $messages = [
        'newFile.required' => 'Por favor, selecione um arquivo.',
        'newFile.mimes' => 'O arquivo deve ser PDF ou CSV.',
        'newFile.max' => 'O arquivo deve ter no máximo 10MB.',
    ...
    public function deleteUploadHistory($uploadId)
    {
        try {
            $upload = CashbookUploadHistory::where('id', $uploadId)
            ...
            if ($upload) {
                ...
                session()->flash('success', 'Histórico excluído com sucesso!');
            }
        } catch (\Exception $e) {
            Log::error('Erro ao excluir histórico', [
                'error' => $e->getMessage(),
                'upload_id' => $uploadId
            ]);
            session()->flash('error', 'Erro ao excluir histórico.');
        }
    }
    ...
    public function updatedNewFile()
    {
        if ($this->newFile) {
            ...
            Log::info('Cashbook file selected', [
                'name' => $this->newFile->getClientOriginalName(),
                'size' => $this->newFile->getSize(),
                'type' => $this->newFile->getClientOriginalExtension()
            ]);
        }
    }
    ...
    public function uploadFile()
    {
        ...
        try {
            ...
            Log::info('Processing cashbook file', [
            ...
            // Salva o arquivo
            $filePath = $this->file->store('cashbook-uploads', 'public');
            ...
            if ($this->file->getClientOriginalExtension() === 'pdf') {
                ...
                Log::info('Transactions extracted from PDF', ['count' => count($transactions)]);

                foreach ($transactions as $i => $t) {
                    $this->applyAutomationsToTransaction($transactions[$i]);
                }
                Log::info('Automations applied to transactions');
            } elseif ($this->file->getClientOriginalExtension() === 'csv') {
                $transactions = $this->parseCsvFile();
                Log::info('Transactions extracted from CSV', ['count' => count($transactions)]);
            ...
            Log::info('Final transactions ready', [
                'count' => count($this->transactions),
                'sample' => count($this->transactions) > 0 ? $this->transactions[0] : null
            ]);
            ...
            Log::error('Error processing cashbook file', [
                'error' => $e->getMessage(),
    ...
    private function applyAutomationsToTransaction(array &$trans)
    {
        ...
        Log::info('Applying automations to transaction', [
            'description' => substr($desc, 0, 80),
            'value' => $trans['value'] ?? null,
            'type_id' => $trans['type_id'] ?? null
        ]);
        ...
        if (!empty($desc) && str_contains($desc, 'eudora') && str_contains($desc, 'dinheiro')){
            Log::info('Detected Eudora transaction');
            ...
            if ($eudoraCof) {
                ...
                Log::info('Mapped Eudora cofrinho', ['cofrinho_id' => $eudoraCof->id]);
            }
            if ($eudoraClient) {
                $trans['client_id'] = $eudoraClient->id;
                Log::info('Mapped Eudora client', ['client_id' => $eudoraClient->id]);
            }
            ...
            if ($pixCat) {
                ...
                Log::info('Mapped PIX category', ['category_id' => $pixCat->id_category]);
            }
        }

        // Ana Carolina -> Aninha
        if (!empty($title) && str_contains($title, mb_strtolower('Ana Carolina Ferreira Coelho Freire'))){
            Log::info('Detected Ana Carolina in title');
        ...
        if ((!empty($title) && str_contains($title, 'transferencia')) || (!empty($desc) && str_contains($desc, 'transferencia'))){
            ...
            if ($pixCat) {
                $trans['category_id'] = $pixCat->id_category ?? null;
                Log::info('Mapped PIX category for transferencia', ['category_id' => $pixCat->id_category]);
            }
        }

        Log::info('Automation result', [
            'category_id' => $trans['category_id'] ?? null,
            'client_id' => $trans['client_id'] ?? null,
            'cofrinho_id' => $trans['cofrinho_id'] ?? null
        ]);
    }
    ...
    public function confirmTransactions()
    {
        ...
        if ($success) {
            ...
            Log::error('Error confirming cashbook transactions', [
                'error' => $e->getMessage(),
                'trace' => $e->getTraceAsString()
            ]);
    ...
    {
        return view('livewire.cashbook.upload-cashbook2');
    }
}
