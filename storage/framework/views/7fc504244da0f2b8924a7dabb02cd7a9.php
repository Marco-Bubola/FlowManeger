<div>
    ...
    <div x-data="{
        modalOpen: true,
        exportType: <?php if ((object) ('exportType') instanceof \Livewire\WireDirective) : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('exportType'->value()); ?>')<?php echo e('exportType'->hasModifier('live') ? '.live' : ''); ?><?php else : ?>window.Livewire.find('<?php echo e($__livewire->getId()); ?>').entangle('<?php echo e('exportType'); ?>')<?php endif; ?>,
        downloading: false,
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                        <div class="space-y-4">
                            ...
                            <div class="space-y-2 pt-4">
                                ...
                                <button @click.prevent="exportSale(<?php echo e($sale->id); ?>, exportType)" :disabled="downloading" class="w-full inline-flex items-center justify-center gap-2 px-5 py-3 bg-gradient-to-r from-blue-500 to-purple-600 text-white font-bold rounded-xl shadow-lg disabled:opacity-50 disabled:cursor-not-allowed">
                        ...
                        <div>
                            ...
                            <div id="export-sale-<?php echo e($sale->id); ?>" style="width: 620px; background: #fff; border-radius: 12px; padding: 18px; box-shadow: 0 12px 32px rgba(0,0,0,0.08); font-family: 'Segoe UI', Tahoma, sans-serif;">
                                <div style="display:flex; justify-content:space-between; align-items:center; margin-bottom:12px;">
                                    <div style="font-weight:800; font-size:18px;">Venda #<?php echo e($sale->id); ?></div>
                                    <div style="font-size:14px; color:#6b7280;"><?php echo e($sale->created_at->format('d/m/Y H:i')); ?></div>
                                </div>

                                <div style="display:flex; gap:14px; align-items:center; margin-bottom:12px;">
                                    <div style="width:64px; height:64px; background:linear-gradient(135deg,#9575cd,#b39ddb); border-radius:12px; display:flex; align-items:center; justify-content:center; color:#fff; font-weight:700;"><?php echo e(strtoupper(substr($sale->client->name ?? 'CL',0,2))); ?></div>
                                    <div>
                                        <div style="font-weight:700;"><?php echo e($sale->client->name ?? 'Cliente'); ?></div>
                                        <div style="color:#6b7280; font-size:13px;">Itens: <?php echo e($sale->saleItems->count()); ?> — Total: R$ <?php echo e(number_format($sale->total_price,2,',','.')); ?></div>
                                    </div>
                                </div>
                                ...
                                <div style="border-top:1px solid #f3f4f6; padding-top:10px;">
                                    <!--[if BLOCK]><![endif]--><?php $__currentLoopData = $sale->saleItems; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $item): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <div style="display:flex; justify-content:space-between; padding:6px 0;">
                                            <div style="min-width:0; overflow:hidden; text-overflow:ellipsis; white-space:nowrap;"><?php echo e($item->product->name ?? 'Produto'); ?></div>
                                            <div style="color:#6b7280;"><?php echo e($item->quantity); ?> x R$ <?php echo e(number_format($item->price_sale,2,',','.')); ?></div>
                                        </div>
                                ...
                                <div style="margin-top:12px; display:flex; justify-content:space-between; font-weight:800;">
                                    ...
                                    <div><?php echo e(ucfirst(str_replace('_',' ', $sale->payment_method ?? ''))); ?></div>
                                </div>
                            </div>
...
<?php /**PATH C:\projetos\FlowManeger\resources\views/livewire/sales/export-sale-modal.blade.php ENDPATH**/ ?>