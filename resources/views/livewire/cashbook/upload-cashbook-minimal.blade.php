<div class=" w-full ">
    ...
    <div class="w-full px-4 sm:px-6 lg:px-8 py-12">
        <div class="hidden" wire:init="ping"></div>
        @if($step == 1)
            <!-- Step 1: Upload File -->
            <div class="w-full bg-gradient-to-br from-white via-blue-50/30 to-purple-50/30 dark:from-gray-800 dark:via-blue-900/30 dark:to-purple-900/30 rounded-3xl shadow-2xl border border-white/20 dark:border-gray-700/50 backdrop-blur-sm">
                ...
                <div class="p-8">
                    <div class="mb-4">
                        <button type="button" wire:click="ping" class="px-3 py-2 bg-amber-100 text-amber-800 rounded">
                            Testar Livewire (ping)
                        </button>
                    </div>
                    @if(session()->has('error'))
                        <div class="mb-4 bg-red-50 border border-red-200 dark:bg-red-900/20 dark:border-red-700 rounded-lg p-4 animate-fadeIn">
                            <div class="flex items-start space-x-3">
                                <svg class="w-5 h-5 text-red-600 dark:text-red-400 mt-0.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L3.732 16.5c-.77.833.192 2.5 1.732 2.5z"></path>
                                </svg>
                                <div class="flex-1">
                                    <h4 class="text-red-800 dark:text-red-300 font-bold mb-1">Erro ao enviar arquivo</h4>
                                    <p class="text-red-700 dark:text-red-300 text-sm whitespace-pre-wrap">{{ session('error') }}</p>
                                </div>
                            </div>
                        </div>
                    @endif

                    <form wire:submit.prevent="processFile" enctype="multipart/form-data" class="space-y-8">
                        <!-- File Upload Zone -->
                        <div class="relative border-3 border-dashed border-blue-300 dark:border-blue-600 rounded-2xl p-12 text-center hover:border-blue-400 dark:hover:border-blue-500 transition-all duration-300 group bg-gradient-to-br from-blue-50/30 via-white to-purple-50/30 dark:from-gray-700/30 dark:via-gray-800/30 dark:to-gray-700/30 hover:shadow-2xl transform hover:scale-[1.02]">
                            ...
                            <div class="relative space-y-6">
                                ...
                                <div class="flex flex-col items-center space-y-2">
                                    <input wire:model="file" wire:key="cashbook-file-input" type="file" id="file"
                                           accept=".pdf,.csv"
                                           class="block w-full max-w-md text-sm text-gray-700 dark:text-gray-200 border border-dashed border-blue-300 dark:border-blue-600 rounded-lg p-3 bg-white dark:bg-gray-800">
                                    <div class="text-xs text-gray-500 dark:text-gray-400" wire:loading wire:target="file">Enviando arquivo...</div>
                                </div>
                        ...
                        <div class="flex justify-center space-x-4">
                            <button type="button" wire:click="cancel"
                                ...
                                Cancelar
                                    ...
                                    <button type="submit" wire:loading.attr="disabled" wire:target="processFile,file"
                                    class="group relative inline-flex items-center px-8 py-3 bg-gradient-to-r from-blue-600 via-purple-600 to-pink-600 dark:from-blue-700 dark:via-purple-700 dark:to-pink-700 text-white font-bold rounded-xl hover:from-blue-700 hover:via-purple-700 hover:to-pink-700 transition-all duration-300 transform hover:scale-105 hover:shadow-2xl shadow-lg disabled:opacity-70 disabled:cursor-not-allowed">
                                ...
                                <span class="relative z-10 ml-2 text-xs" wire:loading wire:target="processFile,file">Enviando...</span>
                            </button>
                        </div>
                    </form>
                </div>
