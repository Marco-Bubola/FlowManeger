<div class="min-h-screen w-full bg-gradient-to-b from-slate-50 via-indigo-50/30 to-slate-50 dark:from-slate-950 dark:via-indigo-950/20 dark:to-slate-950">
    <div class="px-4 sm:px-6 lg:px-8 py-6 space-y-8">
        ...
        <x-sales-header
            ...
            description="Alterações são salvas no sistema e enviadas ao Mercado Livre"
        ...
        {{-- Hero: título da publicação + status + link ML --}}
            ...
            <div class="relative px-6 py-6 sm:px-8 sm:py-8">
                <div class="flex flex-wrap items-start justify-between gap-4">
                    <div class="space-y-2">
                        ...
                        <h2 class="text-xl sm:text-2xl font-bold text-slate-900 dark:text-slate-100 max-w-2xl">{{ $publication->title }}</h2>
                        <div class="flex flex-wrap items-center gap-2">
                            <span class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-slate-200/80 dark:bg-slate-700/80 text-slate-700 dark:text-slate-300 text-xs font-mono">{{ $publication->ml_item_id }}</span>
                            <span class="px-2.5 py-1 rounded-lg text-xs font-semibold {{ $publication->status === 'active' ? 'bg-emerald-500/30 text-emerald-800 dark:text-emerald-200' : '' }} {{ $publication->status === 'paused' ? 'bg-amber-500/30 text-amber-800 dark:text-amber-200' : '' }} {{ $publication->status === 'closed' ? 'bg-red-500/30 text-red-800 dark:text-red-200' : '' }}">{{ ucfirst($publication->status) }}</span>
                            <span class="px-2.5 py-1 rounded-lg bg-purple-500/20 text-purple-700 dark:text-purple-300 text-xs font-semibold">{{ $publicationType === 'kit' ? 'Kit' : 'Simples' }}</span>
                        </div>
                    </div>
                ...
                <div class="rounded-3xl border border-slate-200/80 dark:border-slate-700/80 bg-white/90 dark:bg-slate-900/90 backdrop-blur shadow-xl overflow-hidden">
                    <div class="px-6 py-4 bg-gradient-to-r from-purple-500/10 to-pink-500/10 dark:from-purple-900/30 dark:to-pink-900/30 border-b border-slate-200 dark:border-slate-700 flex flex-wrap items-center justify-between gap-4">
                        <h3 class="text-lg font-bold text-slate-900 dark:text-slate-100 flex items-center gap-2">
                            ...
                            <span class="text-sm font-normal text-slate-500 dark:text-slate-400">({{ count($products) }})</span>
                        </h3>
                    ...
                    <div class="p-6">
                        @if($showProductSelector)
                            <div class="mb-6 p-5 rounded-2xl bg-slate-50 dark:bg-slate-800/50 border-2 border-dashed border-slate-200 dark:border-slate-700">
                                @livewire('mercado-livre.components.product-selector', ['initialProducts' => []], key('selector-' . $publication->id))
                            </div>
                        @endif
                        @if(!empty($products))
                            <div class="space-y-4">
                                @foreach($products as $product)
                                    <div class="flex flex-wrap items-center gap-4 p-5 rounded-2xl bg-gradient-to-r from-slate-50 to-slate-100/80 dark:from-slate-800/80 dark:to-slate-800/50 border border-slate-200 dark:border-slate-700 shadow-sm hover:shadow-md transition-shadow">
                                        ...
                                        <div class="flex-1 min-w-0">
                                            <h4 class="font-semibold text-slate-900 dark:text-slate-100 truncate">{{ $product['name'] }}</h4>
                                            <p class="text-sm text-slate-500 dark:text-slate-400">
                                                {{ $product['product_code'] }} · Estoque: {{ $product['stock_quantity'] }}
                                                @if($publicationType === 'kit')
                                                    · Até {{ floor($product['stock_quantity'] / max(1, $product['quantity'])) }} un. disponíveis
                                                @endif
                                            </p>
                                        </div>
                ...
                <div class="rounded-3xl border border-slate-200/80 dark:border-slate-700/80 bg-white/90 dark:bg-slate-900/90 backdrop-blur shadow-xl overflow-hidden">
                    ...
                    <div class="p-6 space-y-4">
                        <div class="flex justify-between items-center">
                            ...
                            <span class="px-3 py-1.5 rounded-xl text-xs font-semibold {{ $publication->status === 'active' ? 'bg-emerald-500/20 text-emerald-700 dark:text-emerald-300 border border-emerald-400/50' : '' }} {{ $publication->status === 'paused' ? 'bg-amber-500/20 text-amber-700 dark:text-amber-300 border border-amber-400/50' : '' }} {{ $publication->status === 'closed' ? 'bg-red-500/20 text-red-700 dark:text-red-300 border border-red-400/50' : '' }}">{{ ucfirst($publication->status) }}</span>
                        </div>
                        <div class="flex justify-between items-center">
                            <span class="text-sm text-slate-600 dark:text-slate-400">Sincronização</span>
                            <span class="px-3 py-1.5 rounded-xl text-xs font-semibold {{ $publication->sync_status === 'synced' ? 'bg-emerald-500/20 text-emerald-700 dark:text-emerald-300' : '' }} {{ $publication->sync_status === 'pending' ? 'bg-amber-500/20 text-amber-700 dark:text-amber-300' : '' }} {{ $publication->sync_status === 'error' ? 'bg-red-500/20 text-red-700 dark:text-red-300' : '' }}">{{ ucfirst($publication->sync_status) }}</span>
                        </div>
                        <div class="flex justify-between items-center pt-2 border-t border-slate-200 dark:border-slate-700">
                            ...
                            <span class="text-xl font-bold text-indigo-600 dark:text-indigo-400">{{ $availableQuantity }}</span>
                        </div>
                        @if($publication->last_sync_at)
                            <div class="flex justify-between items-center text-sm pt-1">
                                <span class="text-slate-600 dark:text-slate-400">Última sync</span>
                                <span class="text-slate-500 dark:text-slate-400">{{ $publication->last_sync_at->diffForHumans() }}</span>
                            </div>
                        @endif
                        ...
                        @if($stockLogs->count() > 0)
                            <div class="space-y-2">
                                @foreach($stockLogs as $log)
                                    <div class="text-xs p-3 rounded-xl border-l-4 {{ $log->operation_type === 'ml_sale' ? 'bg-red-50 dark:bg-red-900/10 border-red-500' : '' }} {{ $log->operation_type === 'sync_to_ml' ? 'bg-blue-50 dark:bg-blue-900/10 border-blue-500' : '' }} {{ $log->operation_type === 'manual_update' ? 'bg-amber-50 dark:bg-amber-900/10 border-amber-500' : '' }}">
                                        <div class="flex justify-between mb-1">
                                            <span class="font-semibold text-slate-700 dark:text-slate-300">{{ $log->getOperationDescription() }}</span>
                                            <span class="text-slate-500">{{ $log->created_at->format('d/m H:i') }}</span>
                                        </div>
                                        <div class="text-slate-600 dark:text-slate-400">{{ $log->product->name ?? 'Produto' }}: {{ $log->quantity_before }} → {{ $log->quantity_after }} ({{ $log->quantity_change > 0 ? '+' : '' }}{{ $log->quantity_change }})</div>
                                    </div>
                                @endforeach
                            </div>
                        @else
