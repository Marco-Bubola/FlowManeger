{{-- Modal de Export Ultra Moderno (Estilo Produtos) --}}
@if($showExportModal && $selectedClientForExport)
        ...
        <div x-show="true"
            ...
            <div class="relative z-10">
                        ...
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <p class="text-sm text-slate-600 dark:text-slate-400">
                                    Cliente: <span class="font-semibold text-purple-600 dark:text-purple-400">{{ $selectedClientForExport->name }}</span>
                                </p>
                            </div>
                        </div>
                ...
                <div class="px-6 pt-6">
                    <div class="flex gap-2">
                        <button @click="exportType = 'vendas'; selectedSale = null"
                            ...
                            Vendas ({{ $selectedClientForExport->sales->count() }})
                        </button>
                        <button @click="exportType = 'consortiums'; selectedConsortium = null"
                            ...
                            Consórcios ({{ $selectedClientForExport->consortiumParticipants ? $selectedClientForExport->consortiumParticipants->count() : 0 }})
                        </button>
                    </div>
                </div>
                ...
                <div x-show="exportType === 'vendas'" class="p-6 space-y-4">
                    @if($selectedClientForExport->sales->count() > 0)
                        ...
                        <div class="space-y-2 max-h-64 overflow-y-auto pr-2">
                            @foreach($selectedClientForExport->sales as $sale)
                                ...
                                <div class="flex items-center gap-3">
                                    ...
                                    <div>
                                        ...
                                        <p class="text-xs text-slate-600 dark:text-slate-400">
                                            {{ $sale->created_at->format('d/m/Y H:i') }} - R$ {{ number_format($sale->total_price, 2, ',', '.') }}
                                        </p>
                                    </div>
                                </div>
                                <span class="text-xs px-3 py-1 rounded-full font-semibold
                                    {{ $sale->status === 'completed' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400 border border-green-300 dark:border-green-600' : 'bg-yellow-100 text-yellow-700 dark:bg-yellow-900/30 dark:text-yellow-400 border border-yellow-300 dark:border-yellow-600' }}">
                ...
                <div x-show="exportType === 'consortiums'" class="p-6 space-y-4">
                    @if($selectedClientForExport->consortiumParticipants && $selectedClientForExport->consortiumParticipants->count() > 0)
                        <h4 class="text-sm font-semibold text-slate-700 dark:text-slate-300 flex items-center">
                        ...
                        <!-- Lista de Consórcios -->
                        <div class="space-y-2 max-h-64 overflow-y-auto pr-2">
                            @foreach($selectedClientForExport->consortiumParticipants as $participant)
                            <label class="flex items-center justify-between p-4 bg-gradient-to-r from-slate-50 to-green-50 dark:from-slate-700/50 dark:to-green-900/20 hover:from-slate-100 hover:to-green-100 dark:hover:from-slate-700 dark:hover:to-green-900/30 rounded-xl cursor-pointer transition-all border border-slate-200 dark:border-slate-600 hover:border-green-400 dark:hover:border-green-500 shadow-sm hover:shadow-lg">
                                <div class="flex items-center gap-3">
                                    <input type="radio"
                                           ...
                                           @click="selectedConsortium = {{ $participant->id }}"
                                           :checked="selectedConsortium === {{ $participant->id }}"
                                           class="w-4 h-4 text-green-600 focus:ring-green-500 bg-white dark:bg-slate-600 border-slate-300 dark:border-slate-500">
                                    <div>
                                        <p class="text-sm font-semibold text-slate-900 dark:text-white">
                                            {{ $participant->consortium->name ?? 'Consórcio #' . $participant->consortium_id }}
                                        </p>
                                        <p class="text-xs text-slate-600 dark:text-slate-400">
                                            Cota: {{ $participant->quota_number }} -
                                            R$ {{ number_format($participant->consortium->total_value ?? 0, 2, ',', '.') }}
                                        </p>
                                    </div>
                                </div>
                                <span class="text-xs px-3 py-1 rounded-full font-semibold
                                    {{ $participant->status === 'active' ? 'bg-green-100 text-green-700 dark:bg-green-900/30 dark:text-green-400 border border-green-300 dark:border-green-600' : 'bg-slate-100 text-slate-700 dark:bg-slate-700 dark:text-slate-400 border border-slate-300 dark:border-slate-600' }}">
                                    {{ ucfirst($participant->status ?? 'Inativo') }}
                                </span>
                            </label>
                            @endforeach
                        </div>
