            ...
            <div class="flex items-center mb-4">
                ...
                <div>
                    <h1 class="text-3xl font-bold text-gray-900 dark:text-white">
                        <i class="bi bi-person-gear text-indigo-600 dark:text-indigo-400 mr-3"></i>Editar Cliente
                    </h1>
                    <p class="mt-2 text-sm text-gray-600 dark:text-gray-400">Atualize as informações do cliente {{ $client->name }}</p>
                </div>
            </div>
        </div>
                        ...
                        @foreach($this->avatarOptions as $index => $avatar)
                            <label class="relative cursor-pointer group">
                                <input type="radio" 
                                       ...
                                       value="{{ $avatar }}" 
                                       class="sr-only peer">
                                <div class="w-16 h-16 rounded-full border-4 border-gray-200 dark:border-zinc-600 overflow-hidden peer-checked:border-indigo-500 peer-checked:ring-2 peer-checked:ring-indigo-200 dark:peer-checked:ring-indigo-400 group-hover:border-indigo-300 transition-all duration-200">
                                    <img src="{{ $avatar }}" 
                                         alt="Avatar Option {{ $index + 1 }}"
                                         class="w-full h-full object-cover"
                                         loading="lazy">
                                </div>
                                <div class="absolute inset-0 rounded-full bg-indigo-600 bg-opacity-0 peer-checked:bg-opacity-20 transition-all duration-200"></div>
                                <i class="bi bi-check absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 text-white opacity-0 peer-checked:opacity-100 transition-opacity duration-200 text-lg font-bold"></i>
                            </label>
                        @endforeach
                    </div>
