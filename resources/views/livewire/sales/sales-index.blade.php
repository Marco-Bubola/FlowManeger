...
<div class="w-full" x-data="{
    ...
    <div class="">
        ...
        <!-- Header Moderno -->
        <x-sales-index-header title="Vendas" :total-sales="$totalSales ?? 0" :pending-sales="$pendingSales ?? 0" :today-sales="$todaySales ?? 0" :total-revenue="$totalRevenue ?? 0"
            :show-quick-actions="true" />

        <!-- Barra de Controle Superior com Pesquisa e Paginação -->
        <div
            class="bg-white/70 dark:bg-slate-800/70 backdrop-blur-xl rounded-2xl p-4 shadow-lg border border-white/20 dark:border-slate-700/50 mb-6">
                    ...
                    <div class="relative group">
                        ...
                        <div class="relative">
                            ...
                            <div class="absolute left-4 top-1/2 transform -translate-y-1/2">
                                <i
                                    class="bi bi-search text-slate-500 dark:text-slate-400 text-lg group-focus-within:text-purple-500 transition-colors duration-200"></i>
                            </div>
                            ...
                            <div class="absolute right-3 top-1/2 transform -translate-y-1/2">
                                <button wire:click="$set('search', '')" x-show="$wire.search && $wire.search.length > 0"
                                    ...
                                    title="Limpar busca">
                                    <i
                                        class="bi bi-x-lg text-xs group-hover/clear:rotate-90 transition-transform duration-200"></i>
                                </button>
                            </div>
                    ...
                    @if ($sales->hasPages())
                        <div class="flex items-center gap-1 bg-slate-100 dark:bg-slate-700 rounded-lg p-1">
                            ...
                            @if ($sales->currentPage() < $sales->lastPage())
                                <button wire:click.prevent="gotoPage({{ $sales->lastPage() }})"
