    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        @if($banks && count($banks) > 1)
            <div class="bg-white p-4 rounded-xl shadow-sm border border-gray-200 mb-6">
                <div class="flex items-center justify-between">
                    <label for="bank-selector" class="text-sm font-medium text-gray-700">Selecionar Banco:</label>
                    <select wire:change="changeBank($event.target.value)" id="bank-selector" 
                        ...
                        @foreach($banks as $bankOption)
                            <option value="{{ $bankOption->id_bank }}" {{ $bankOption->id_bank == $bankId ? 'selected' : '' }}>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm font-medium text-gray-600">Total do Mês</p>
                        <p class="text-2xl font-bold text-gray-900">R$ {{ number_format($totalInvoices, 2, ',', '.') }}</p>
                    </div>
            ...
            <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm font-medium text-gray-600">Maior Valor</p>
            ...
            <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-200">
                <div class="flex items-center justify-between">
                    <div>
                        <p class="text-sm font-medium text-gray-600">Menor Valor</p>
                        <p class="text-2xl font-bold text-gray-900">
        ...
        <div class="bg-white p-6 rounded-xl shadow-sm border border-gray-200 mb-8">
            <div class="flex items-center justify-between">
                ...
                <div class="text-center">
                    <h2 class="text-xl font-bold text-gray-900">{{ $currentMonthName }}</h2>
            ...
            <div class="overflow-hidden">
                                ...
                                <div class="flex items-center justify-between">
                                        ...
                                        <div>
                                            ...
                                            <p class="text-sm text-gray-500 flex items-center">
                                                ...
                                                @if($invoice->installments)
                                                    <span class="ml-2 text-xs bg-gray-100 text-gray-800 px-2 py-1 rounded">
                                    ...
                                    <div class="flex items-center space-x-4">
                                        <div class="text-right">
                                            <p class="text-sm font-medium text-gray-900">R$ {{ number_format($invoice->value, 2, ',', '.') }}</p>
                                            @if($invoice->category)
                                                <p class="text-xs text-gray-500">{{ $invoice->category->name }}</p>
                                            @endif
                                        </div>
                                        
                                        @if($bankId)
                                            <div class="flex items-center space-x-2">
                                                <a href="{{ route('invoices.edit', $invoice->id_invoice ?? $invoice->id) }}" 
                                                ...
                                                <a href="{{ route('invoices.copy', $invoice->id_invoice ?? $invoice->id) }}" 
                                                ...
                                                <button wire:click="confirmDelete({{ $invoice->id_invoice ?? $invoice->id }})" 
                ...
                @elseif($bankId)
                    <div class="px-6 py-12 text-center">
                        ...
                        <p class="text-gray-500 mb-4">Não há transações para o período selecionado.</p>
                        <a href="{{ route('invoices.create', ['bankId' => $bankId]) }}" 
                           class="inline-flex items-center px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 transition-colors duration-200">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 6v6m0 0v6m0-6h6m-6 0H6"></path>
                ...
                @else
                    <div class="px-6 py-12 text-center">
                        ...
                        <h3 class="text-lg font-medium text-gray-900 mb-2">Selecione um banco</h3>
                        <p class="text-gray-500 mb-4">Para visualizar as transações, você precisa primeiro selecionar um banco.</p>
                        <a href="{{ route('banks.index') }}" 
                           class="inline-flex items-center px-4 py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 transition-colors duration-200">
                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"></path>
        ...
        <div class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
            <div class="relative top-20 mx-auto p-5 border w-96 shadow-lg rounded-md bg-white">
                <div class="mt-3 text-center">
                    ...
                    <h3 class="text-lg font-medium text-gray-900 mt-2">Excluir Transação</h3>
