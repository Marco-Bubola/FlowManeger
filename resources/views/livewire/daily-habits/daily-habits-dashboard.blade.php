<div class="w-full bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900" style="min-height: 100vh;">
    ...
    <x-sales-header
        ...
        description="Construa sua melhor versão, um dia de cada vez">
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div class="mb-6 p-4 bg-gradient-to-r from-green-400 to-emerald-500 text-white rounded-xl shadow-lg flex items-center gap-3 animate-fade-in">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            <!-- Total Hábitos -->
            <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-xl p-6 border border-slate-200 dark:border-slate-700 hover:scale-105 transition-transform duration-200">
                <div class="flex items-center justify-between mb-3">
                    <div class="flex items-center justify-center w-14 h-14 bg-gradient-to-br from-blue-400 to-blue-600 rounded-xl shadow-lg">
                        <i class="bi bi-list-check text-2xl text-white"></i>
                    </div>
                    <span class="text-4xl font-black text-slate-800 dark:text-white">{{ $stats['total_habits'] }}</span>
                </div>
                <h3 class="text-slate-600 dark:text-slate-400 font-semibold">Hábitos Ativos</h3>
            </div>

            <!-- Concluídos Hoje -->
            <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-xl p-6 border border-slate-200 dark:border-slate-700 hover:scale-105 transition-transform duration-200">
                <div class="flex items-center justify-between mb-3">
                    <div class="flex items-center justify-center w-14 h-14 bg-gradient-to-br from-green-400 to-emerald-600 rounded-xl shadow-lg">
                        <i class="bi bi-check-circle-fill text-2xl text-white"></i>
                    </div>
                    <span class="text-4xl font-black text-slate-800 dark:text-white">{{ $stats['completed_today'] }}</span>
                </div>
                <h3 class="text-slate-600 dark:text-slate-400 font-semibold">Concluídos Hoje</h3>
                <div class="mt-2 bg-slate-100 dark:bg-slate-700 rounded-full h-2 overflow-hidden">
                ...
                <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">{{ $stats['completion_percentage'] }}% completado</p>
            </div>

            <!-- Semana -->
            <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-xl p-6 border border-slate-200 dark:border-slate-700 hover:scale-105 transition-transform duration-200">
                <div class="flex items-center justify-between mb-3">
                    <div class="flex items-center justify-center w-14 h-14 bg-gradient-to-br from-orange-400 to-amber-600 rounded-xl shadow-lg">
                        <i class="bi bi-calendar-week text-2xl text-white"></i>
                    </div>
                    <span class="text-4xl font-black text-slate-800 dark:text-white">{{ $stats['week_completions'] }}</span>
                </div>
                <h3 class="text-slate-600 dark:text-slate-400 font-semibold">Esta Semana</h3>
            </div>

            <!-- Melhor Sequência -->
            <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-xl p-6 border border-slate-200 dark:border-slate-700 hover:scale-105 transition-transform duration-200">
                <div class="flex items-center justify-between mb-3">
                    <div class="flex items-center justify-center w-14 h-14 bg-gradient-to-br from-purple-400 to-pink-600 rounded-xl shadow-lg">
                        <i class="bi bi-fire text-2xl text-white"></i>
                    </div>
                    <span class="text-4xl font-black text-slate-800 dark:text-white">{{ $stats['best_streak'] }}</span>
                </div>
        ...
        <div class="animate-bounce-in mb-6">
            <div class="bg-gradient-to-r {{ session('message_type') === 'error' ? 'from-red-500 to-red-600' : 'from-green-500 to-emerald-600' }} text-white px-6 py-4 rounded-2xl shadow-lg flex items-center gap-3">
                <i class="bi {{ session('message_type') === 'error' ? 'bi-x-circle-fill' : 'bi-check-circle-fill' }} text-2xl"></i>
                <span class="font-semibold">{{ session('message') }}</span>
            </div>
        </div>
        @endif
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 pb-8">
            ...
            <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-xl overflow-hidden border-2 transition-all duration-300 hover:shadow-2xl hover:scale-105 {{ $habit['is_completed_today'] ? 'border-green-400' : 'border-slate-200 dark:border-slate-700' }}">
                <!-- Header do Card -->
                <div class="p-6" style="background: linear-gradient(135deg, {{ $habit['color'] }}20, {{ $habit['color'] }}10);">
                    <div class="flex items-start justify-between mb-4">
                        <!-- Ícone e Nome -->
                        <div class="flex items-center gap-4 flex-1">
                            <div class="flex items-center justify-center w-16 h-16 rounded-2xl shadow-lg" style="background: {{ $habit['color'] }};">
                                <i class="bi {{ $habit['icon'] }} text-3xl text-white"></i>
                            </div>
                            <div class="flex-1">
                                <h3 class="text-xl font-bold text-slate-800 dark:text-white mb-1">{{ $habit['name'] }}</h3>
                    ...
                    <button wire:click="toggleHabit({{ $habit['id'] }})"
                        ...
                        <span wire:loading wire:target="toggleHabit({{ $habit['id'] }})" class="flex items-center justify-center gap-2">
        ...
        <div class="flex flex-col items-center justify-center py-20">
            <div class="flex items-center justify-center w-32 h-32 bg-gradient-to-br from-indigo-100 to-purple-100 dark:from-indigo-900/30 dark:to-purple-900/30 rounded-full mb-6">
