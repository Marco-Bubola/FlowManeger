    ...
    <div class="w-full bg-gradient-to-br from-indigo-50 via-purple-50 to-pink-50 dark:from-slate-900 dark:via-slate-800 dark:to-slate-900"
        ...
        <div class="max-w-4xl mx-auto px-6 py-8">
            ...
            @if (session()->has('message'))
                <div
                    class="mb-6 p-4 bg-gradient-to-r from-green-500 to-emerald-600 text-white rounded-xl shadow-lg animate-bounce-in">
                    {{ session('message') }}
                </div>
            @endif
            ...
            @if (session()->has('error'))
                <div
                    class="mb-6 p-4 bg-gradient-to-r from-red-500 to-rose-600 text-white rounded-xl shadow-lg animate-bounce-in">
                    {{ session('error') }}
                </div>
            @endif
                ...
                <form wire:submit.prevent="updateHabit">
                    ...
                    <div class="mb-6">
                        ...
                        <textarea wire:model="description" rows="3"
                            class="w-full px-4 py-3 bg-slate-50 dark:bg-slate-700 border border-gray-300 dark:border-slate-600 rounded-xl focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all text-gray-900 dark:text-white"
                            placeholder="Descreva seu hábito..."></textarea>
                        @error('description')
                            <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
                        @enderror
                    </div>
                    ...
                    <div class="mb-6">
                        ...
                        <div class="flex flex-wrap gap-3">
                            @foreach ($availableColors as $colorHex => $colorName)
                                <button type="button" wire:click="$set('color', '{{ $colorHex }}')"
...
<span class="text-red-500
                    ...
                    <div class="mb-6">
                        ...
                        <input type="number" wire:model="goal_frequency" min="1" max="10"
                        ...
                        <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Meta de quantas vezes você quer
                            fazer este hábito por dia (1-10)</p>
                    ...
                    <div class="mb-8">
                        ...
                        <input type="time" wire:model="reminder_time"
                            class="w-full px-4 py-3 bg-slate-50 dark:bg-slate-700 border border-gray-300 dark:border-slate-600 rounded-xl focus:ring-2 focus:ring-blue-500 text-gray-900 dark:text-white focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all">
                        <p class="text-sm text-gray-500 dark:text-gray-400 mt-1">Receba uma notificação neste
                            horário para lembrar do seu hábito</p>
                        @error('reminder_time')
                            <span class="text-red-500 text-sm mt-1">{{ $message }}</span>
                        @enderror
                    </div>
            ...
            <div class="flex items-center gap-4 p-4 rounded-xl border-2 border-gray-200 dark:border-slate-600">
                ...
                <div class="flex-1">
                    <h4 class="font-semibold text-gray-800 dark:text-gray-200\">{{ $name }}</h4>
                        @if ($description)
<p class="text-sm
                        text-gray-600 mt-1">{{ $description }}</p>
    @endif
...
<div class="bg-white dark:bg-slate-800 rounded-2xl shadow-2xl max-w-md w-full p-6 animate-bounce-in">
    <div class="flex items-center gap-3 mb-4">
        ...
        <h3 class="text-xl font-bold text-gray-800 dark:text-gray-200\">Confirmar Arquivamento</h3>
            ...
            Tem certeza que deseja arquivar este hábito? Ele será removido da sua lista ativa, mas os dados
            serão mantidos.
            </p>
