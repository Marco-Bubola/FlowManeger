        ...
        <div class="p-2 space-y-2.5">
            @foreach($selectedProducts as $index => $productItem)
                @if(isset($productItem['id']) && $productItem['id'])
                    ...
                    <div class="{{ $modernStyle ? 'bg-gradient-to-br from-white via-purple-50/40 to-indigo-50/40 dark:from-slate-800 dark:via-slate-700/40 dark:to-slate-800/60' : 'bg-white dark:bg-zinc-800' }} rounded-xl p-2.5 shadow-sm border border-gray-200 dark:border-zinc-700 overflow-hidden">
                        <!-- Header do produto -->
                        <div class="flex items-start justify-between mb-2">
                            <div class="flex items-center space-x-2 min-w-0">
                                <img src="{{ ($selectedProduct['image'] ?? null) ? asset('storage/products/' . $selectedProduct['image']) : asset('storage/products/product-placeholder.png') }}"
                                     alt="{{ $selectedProduct['name'] ?? 'Produto' }}"
                                      class="w-10 h-10 md:w-12 md:h-12 rounded-lg object-cover border border-slate-200 dark:border-slate-600 shrink-0">

                                <div class="flex-1 min-w-0">
                                    <h4 class="font-semibold text-gray-900 dark:text-white text-xs truncate">{{ $selectedProduct['name'] ?? 'Produto' }}</h4>
                                    <div class="flex items-center gap-1.5 mt-1">
                                        <span class="inline-flex items-center px-1.5 py-0.5 rounded-full text-[10px] font-semibold bg-indigo-100 text-indigo-700 dark:bg-indigo-900/40 dark:text-indigo-300">
                                            <i class="bi bi-tag mr-1"></i>R$ {{ number_format($productItem['price'] ?? 0, 2, ',', '.') }}
                                        </span>
                                        <span class="inline-flex items-center px-1.5 py-0.5 rounded-full text-[10px] font-semibold bg-purple-100 text-purple-700 dark:bg-purple-900/40 dark:text-purple-300">
                                            <i class="bi bi-currency-dollar mr-1"></i>R$ {{ number_format($productItem['salePrice'] ?? 0, 2, ',', '.') }}
                                        </span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <button type="button"
                                    ...
                                    class="text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300 p-1.5 hover:bg-red-50 dark:hover:bg-red-900/30 rounded-lg transition-colors duration-200">
                            ...
                            <div class="grid grid-cols-2 gap-2">
                                ...
                                <div>
                                    ...
                                    <input type="number"
                                           ...
                                           class="w-full px-2 py-1.5 text-xs border border-gray-300 dark:border-zinc-600 rounded-lg bg-white dark:bg-zinc-700 text-gray-900 dark:text-white focus:ring-1 focus:ring-purple-500 focus:border-transparent">
                                </div>
                                @endif
                            ...
                            <div class="pt-1 border-t border-gray-200 dark:border-zinc-600">
                                <div class="flex justify-between items-center">
                                    ...
                                    <span class="text-xs font-bold text-green-600 dark:text-green-400">
                                        R$ {{ number_format(($productItem['quantity'] ?? 1) * ($productItem['salePrice'] ?? 0), 2, ',', '.') }}
                                    </span>
                                </div>
                            </div>
                            @endif
