@php
    ...
    $hScore      = collect([$hVerified, !empty($hUser->password), $hHasPhone, $hHasAvatar])->filter()->count();
    $hScorePct   = intval($hScore / 4 * 100);
    $hScoreClr   = $hScorePct >= 75 ? '#10b981' : ($hScorePct >= 50 ? '#f59e0b' : '#ef4444');
    $hScoreLbl   = $hScorePct >= 75 ? 'Boa' : ($hScorePct >= 50 ? 'Razoável' : 'Fraca');
    $hCurrentPage = request()->routeIs('settings.*') ? last(explode('.', request()->route()->getName())) : 'profile';
    $hPageTitles  = [
        'profile'       => 'Perfil',
        'password'      => 'Senha & Login',
        'security'      => 'Segurança',
        'appearance'    => 'Aparência',
        'system'        => 'Sistema & Região',
        'notifications' => 'Notificações',
        'devices'       => 'Dispositivos',
        'plan'          => 'Plano & Assinatura',
        'connections'   => 'Conexões',
    ];
    $hCurrentTitle = $hPageTitles[$hCurrentPage] ?? 'Configurações';
@endphp
